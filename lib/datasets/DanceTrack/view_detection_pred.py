...
def main(args):
    ...
    pred_list = [{x.split('.')[0]: x for x in os.listdir(pred_dir) if x.endswith('.txt')}
    ...
    for vid_idx, vid_name in enumerate(vid_list):
        ...
        preds = [parsing_dance_detection(pred_path) for pred_path in pred_paths]
        ...
        while img_idx < total_len:
            ...
            for j, (detector_name, pred) in enumerate(zip(pred_dirs.keys(), preds)):
                ...
                for bbox in bboxes:
                    ...
                    plot_label(tmp_img, xyxy, f'{cls}: {conf:.2f}', cls_color, font_size=0.7, font_thickness=2)
...
def parse_args():
    ...
    pred_dirs = {
        # 'coco': '/home/jhc/PycharmProjects/MOT/MOT_study/ConfTrack/results/detector/yolox/DanceTrack_val/yolox_x_coco_custom',
        'yolox': '/home/jhc/PycharmProjects/MOT/MOT_study/ConfTrack/results/detector/yolox/DanceTrack_val/yolox_x_ocsort_dance_custom'
    }
