            ...
            if not single_img:
                ...
                for j, (tracker_name, pred) in enumerate(zip(pred_dirs.keys(), preds)):
                    ...
                    for bbox in bboxes:
                        ...
                        xyxy = xywh2xyxy(xywh)
                        cv2.rectangle(tmp_img, (int(xyxy[0]), int(xyxy[1])), (int(xyxy[2]), int(xyxy[3])), trk_color, 3)
                ...
                for j, (tracker_name, pred) in enumerate(zip(pred_dirs.keys(), preds)):
                    ...
                    for bbox in bboxes:
                        ...
                        xywh = np.array(list(map(float, bbox[1: 5])))
                        xyxy = xywh2xyxy(xywh)
                        cv2.rectangle(pred_img, (int(xyxy[0]), int(xyxy[1])), (int(xyxy[2]), int(xyxy[3])), trk_color, 3)
...
def parse_args():
    ...
    pred_dirs = {
        ...
        # 'baseline_nonclip': '/home/jhc/PycharmProjects/MOT/MOT_study/ConfTrack/results/tracker/ConfTrack_ablation_round1/MOT20_val/baseline/MOT20-val/ConfTrack_ablation_round1/data'

        # 'sub2': '/home/jhc/PycharmProjects/MOT/MOT_study/ConfTrack/submission/mot20/submission_v2_custom_det_interp/MOT20-test/ConfTrack_test/data',
        # 'sub3': '/home/jhc/PycharmProjects/MOT/MOT_study/ConfTrack/submission/mot20/all_clipping_interp/MOT20-test/ConfTrack_ablation_round1/data'
